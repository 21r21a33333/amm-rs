...
impl Executable for UniswapV2Pool {
    ...
    fn build_swap_exact_out(
        ...
        let r = common::resolve_swap(ctx, self, from, amount_out.currency, opts)?;
...
mod tests {
    ...
    fn build_swap_exact_out_exact_in_route_returns_unsupported_protocol() {
        // V2 exact-out with a price_limit must return UnsupportedProtocol (V2 has no sqrtPriceLimit).
        use amm_core::primitives::price::Price;
        use amm_core::primitives::ratio::Ratio;
        ...
        let err = p
            ...
            .expect_err("price_limit on V2 exact-out must return UnsupportedProtocol");
